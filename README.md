...
The Siam Protocol is an Binance Smart Chain smart contract for supplying or borrowing assets. Through the sToken contracts, accounts on the blockchain *supply* capital (BNB or BEP-20 tokens) to receive sTokens or *borrow* assets from the protocol (holding other assets as collateral). The protocol will also enable the minting of TAI, which is the first synthetic stablecoin on Siam that aims to be pegged to 1 THB. TAI is minted by the same collateral that is supplied to the protocol. The Siam sToken contracts track these balances and algorithmically set interest rates for borrowers.
...
We detail a few of the core contracts in the Siam protocol.

<dl>
  <dt>SToken, SBep20 and SBNB</dt>
  <dd>The Siam sTokens, which are self-contained borrowing and lending contracts. SToken contains the core logic and SBep20, SBUSD, SSIAM and SBNB add public interfaces for Bep20 tokens and bnb, respectively. Each SToken is assigned an interest rate and risk model (see InterestRateModel and Comptroller sections), and allows accounts to *mint* (supply capital), *redeem* (withdraw capital), *borrow* and *repay a borrow*. Each SToken is an BEP-20 compliant token where balances represent ownership of the market.</dd>
</dl>
...
<dl>
  <dt>SIAM</dt>
  <dd>The Siam Governance Token (SIAM). Holders of this token have the ability to govern the protocol via the governor contract.</dd>
</dl>
...
<dl>
  ...
  <dd>The administrator of the Siam timelock contract. Holders of SIAM token may create and vote on proposals which will be queued into the Siam timelock and then have effects on Siam sToken and Comptroller contracts. This contract may be replaced in the future with a beta version.</dd>
</dl>
...
Installation
------------
To run siam, pull the repository from GitHub and install its dependencies. You will need [yarn](https://yarnpkg.com/lang/en/docs/install/) or [npm](https://docs.npmjs.com/cli/install) installed.
...
The Siam Protocol has a simple scenario evaluation tool to test and evaluate scenarios which could occur on the blockchain. This is primarily used for constructing high-level integration tests. The tool also has a REPL to interact with local the Venus Protocol (similar to `truffle console`).
